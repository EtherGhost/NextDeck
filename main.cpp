...
#ifndef NEXTDECK_VERSION
#define NEXTDECK_VERSION "development"
#endif
...
QHash<QString, QString> readDesktopTestEnvFile()
{
    ...
    candidates << QDir::current().filePath(QStringLiteral(".clickable/nextdeck-desktop-env.local"));
    candidates << QDir::current().filePath(QStringLiteral(".env.test.local"));

    QDir appDir(QCoreApplication::applicationDirPath());
    for (int i = 0; i < 6; ++i) {
        candidates << appDir.filePath(QStringLiteral(".clickable/nextdeck-desktop-env.local"));
...
bool configBool(const QHash<QString, QString> &fileValues, const char *name, bool defaultValue)
{
    const QString value = configValue(fileValues, name).toLower();
    if (value.isEmpty()) {
        return defaultValue;
    }
...
int main(int argc, char *argv[])
{
    ...
    QSettings appSettings(QStringLiteral("nextdeck.cloudsite"), QStringLiteral("nextdeck.cloudsite"));
    ...
    app.setApplicationName(QStringLiteral("nextdeck.cloudsite"));

    const QHash<QString, QString> desktopEnv = desktopLarge ? readDesktopTestEnvFile() : QHash<QString, QString>();
    const bool desktopDarkMode = desktopLarge && configBool(desktopEnv, "NEXTDECK_DESKTOP_DARK_MODE", true);
    const QString desktopTestServer = firstConfigValue(desktopEnv, {"NEXTDECK_TEST_SERVER", "NEXTCLOUD_TEST_SERVER", "NEXTNEWS_TEST_SERVER", "NEXTNOTES_TEST_SERVER"});
    const QString desktopTestUserName = firstConfigValue(desktopEnv, {"NEXTDECK_TEST_USERNAME", "NEXTCLOUD_TEST_USERNAME", "NEXTNEWS_TEST_USERNAME", "NEXTNOTES_TEST_USERNAME"});
    const QString desktopTestSecret = firstConfigValue(desktopEnv, {"NEXTDECK_TEST_APP_PASSWORD", "NEXTCLOUD_TEST_APP_PASSWORD", "NEXTNEWS_TEST_APP_PASSWORD", "NEXTNOTES_TEST_APP_PASSWORD"});
    const bool desktopTestAuthEnabled = desktopLarge
        && firstConfigValue(desktopEnv, {"NEXTDECK_DESKTOP_TEST_AUTH", "NEXTCLOUD_DESKTOP_TEST_AUTH", "NEXTNEWS_DESKTOP_TEST_AUTH", "NEXTNOTES_DESKTOP_TEST_AUTH"}) == QStringLiteral("1")
        && !desktopTestServer.isEmpty()
        && !desktopTestUserName.isEmpty()
        && !desktopTestSecret.isEmpty();

    QQuickView view;
    view.rootContext()->setContextProperty(QStringLiteral("nextdeckAppVersion"), QStringLiteral(NEXTDECK_VERSION));
    ...
    DeckNetwork deckNetwork;
    ContentHubBridge contentHubBridge;
    view.rootContext()->setContextProperty(QStringLiteral("deckNetwork"), &deckNetwork);
    ...
    qInfo("NextDeck desktopLarge=%s desktopDarkMode=%s desktopTestAuth=%s",

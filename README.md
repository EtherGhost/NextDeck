...
Known limitations: the offline-first queue currently covers cards, not board/list management or attachment operations. Conflict handling marks cards as conflicted but does not provide a full merge UI yet. Attachment opening depends on installed Ubuntu Touch apps that can receive the downloaded document through Content Hub. NextDeck is still early software; bugs and rough edges are expected.
